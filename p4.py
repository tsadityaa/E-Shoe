...
def cnt_teller(f):
    while True:
        if f.stepPrev < f.step_cnt:
            sq.put(f"{f.step_cnt}  steps covered,")
        time.sleep(10000)
...
class UltrasonicSensorThread():
    ...
    def runner(self):
        ...
        pwm.start(7.5)
        d = {0: "e", 45: "N E", 90: "N", 135: "NW", 180: "w"}
...
class neo6m(threading.Thread):
    ...
    def curraddress(self):
        latitude, longitude = self.al[2]
        if latitude is not None and longitude is not None:
            self.sq.put(self.get_address(latitude, longitude))
            return self.get_address(latitude, longitude)
        else:

            self.sq.put(self.get_address(16.4963, 80.5007))
            return self.get_address(16.4963, 80.5007)

    def send_message(self):
        bot = telebot.TeleBot(TOKEN)

        message = f"its emergency! /n {self.get_address(self.al[2][0], self.al[2][1])} /n https://www.google.com/maps?q={self.latitude / 100},{self.longitude / 100}"
        for i in CHAT_ID:
...
def main():
    ...
    cntteller_thread = threading.Thread(target=cnt_teller, args=[fsr_thread])
    cntteller_thread.start()
